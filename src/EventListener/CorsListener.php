...
final class CorsListener implements EventSubscriberInterface
{
    ...
    public function onKernelException(ExceptionEvent $event): void
    {
        ...
        if ($response) {
            $response->headers->set('Access-Control-Allow-Origin', 'http://localhost:9527');
            $response->headers->set('Access-Control-Allow-Methods', 'GET,POST,PUT,PATCH');
            $response->headers->set('Access-Control-Allow-Headers', 'Authorization, X-Requested-With, X-HTTP-Method-Override, Content-Type, Accept');
